...
import org.webant.worker.link.{H2LinkProvider, Progress, SqlitelLinkProvider}
...
class SqliteTest extends AssertionsForJUnit {
  ...
  @Test
  def testGroupByLink(): Unit = {
    val provider = new H2LinkProvider

    val params = new util.HashMap[String, Object]()
    params.put("url", "jdbc:h2:D:/workspace/webant/data/h2/webant;MODE=MYSQL")
    params.put("username", "webant")
    params.put("password", "webant")

    provider.init(params)
    val progress = provider.progress()
